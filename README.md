...
## Abstract (Informal)
Q1. How to rigorously define the process of problem-solving?
A1. As a MDP.

Q2. Beyond proving known targets, can we use existing formal theorem proving (FTP) to conduct process-level verified problem-solving, e.g. computation, equation solving, and counter-example construction?
A2. Yes, we can.
...
*For Proof Search and Whole-Proof Generation, setting `only_proving=False` runs FPS (Finding a correct answer + Proving its correctness), while `only_proving=True` runs FTP (Proving the correctness of ground-truth answer).

### Proof Search (FPS)
- `model=stepprover_vanilla_avggoal`: Best-first Search using InternLM2.5-StepProver
- `model=stepprover_critic_avggoal`: Best-first Search using InternLM2.5-StepProver + Critic Model
- `model=leanstar_vanilla_avggoal`: Best-first Search using LeanSTaR
...
do
    python -m evaluator.fps_proof_search \
            ...
            --critic_base_url "https://url.to.openai-style.api.server/" \
            --critic_api_key "critic_api_key" \
            --critic_model_name "name/of/critic-model" \
            --only_proving False
done
```
...
for FPS_BENCHMARK in formal_math500 minif2f_solving putnam_solving;
do
    python -m evaluator.fps_whole_proof_generation_v2 \
            --log_root /path/to/output \
            --model "model_to_evaluate" \
            --benchmark ${FPS_BENCHMARK} \
            --project_root /path/to/mathlib4 \
...
- In-Context Learning: DeepSeek-V3
```shell
ulimit -s unlimited
for FPS_BENCHMARK in formal_math500 minif2f_solving putnam_solving;
do
    ...
    python -m evaluator.fps_icl \
            ...
            --gen_api_key "api-key" \
            --gen_model_name "deepseek-chat"
done
```

- Hybrid CoT: DeepSeek-V3
```shell
ulimit -s unlimited
for FPS_BENCHMARK in formal_math500 minif2f_solving putnam_solving;
do
